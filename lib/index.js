 ...
 * 
 * @option max-age, number or object containing <code>milliseconds</code>,
...
module.exports.getSTS = function(options) {
  ...
  if (typeof options["max-age"] === "number") {
    ...
    if (typeof options["max-age"].milliseconds === "number") {
      max_age += options["max-age"].milliseconds;
    }
    if (typeof options["max-age"].seconds === "number") {
      max_age += options["max-age"].seconds * 1000;
    }
    if (typeof options["max-age"].minutes === "number") {
      max_age += options["max-age"].minutes * 60 * 1000;
    }
    if (typeof options["max-age"].hours === "number") {
      max_age += options["max-age"].hours * 60 * 60 * 1000;
    }
    if (typeof options["max-age"].days === "number") {
      max_age += options["max-age"].days * 24 * 60 * 60 * 1000;
    }
  } else {
    max_age = 1000 * 60 * 60 * 30; // 30 days default
  }
